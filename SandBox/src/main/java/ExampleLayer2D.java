...
public class ExampleLayer2D extends Layer {
    ...
    public void onUpdate(TimeStep timeStep) {
        ...
        Renderer2D.drawQuad(new Vector3f(0.0f, 0.0f, 0.1f), new Vector2f(4.0f, 4.0f), bricks, 20.0f);
        Renderer2D.drawRotatedQuad(new Vector3f(2.0f, 2.0f, 0.3f), new Vector2f(1.0f, 1.0f), 45, dirt, 3.0f);
        Renderer2D.drawQuad(new Vector3f(-1.0f, 2.0f, 0.3f), new Vector2f(1.0f, 1.0f), dirt);

        double time = glfwGetTime();
        for (int i = 0; i < 117; i++) {
            for (int j = 0; j < 117; j++) {
                Renderer2D.drawQuad(new Vector3f(-2.0f + (i * 0.01f), -2.0f + (j * 0.01f), 0.3f), new Vector2f(0.009f, 0.009f), new Vector4f((float) i / 114, 0.4f, (float) j / 114, 0.5f));
            }
        }
